...
std::string __CalTimeZoneImpl::__getTzByOffsetFromWordclock(int offset) const
{
	...
	do
	{
		...
		snprintf(query, DATETIME_BUFFER * 4, CAL_WORLDCLOCK_DB_TIMEZONE_QUERY, offset);
...
CalTimeZone::CalTimeZone(const char *tz)
{
	...
	{
		__tz = TIMEZONE_UNKNOWN;
	}
	__pv = std::make_shared<__CalTimeZoneImpl>(__tz);
	__offset = __pv->getOffset();
}
...
CalTimeZone::CalTimeZone(const CalTimeZone& obj)
{
	__tz = obj.__tz;
	__pv = obj.__pv;
	__offset = obj.__offset;
}

CalTimeZone::CalTimeZone(CalTimeZone& obj)
{
	__tz = obj.__tz;
	__pv = obj.__pv;
	__offset = obj.__offset;
...
std::string CalTimeZone::getSting(LabelType typeFormat) const
{
	switch (typeFormat)
	{
