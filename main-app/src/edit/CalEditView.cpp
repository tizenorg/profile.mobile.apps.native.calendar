			...
			CalAppControlLauncher::getInstance().sendLaunchRequest(service,
				[](app_control_h request, app_control_h reply, app_control_result_e result, void *data)
				{
					...
					if (timezone &&  strlen(timezone) > 0) {
						...
						WDEBUG("AAAAAA1 timezone: %s", dT.c_str());
					}
					...
					WDEBUG("AAAAAA2 timezone: %s", tz.c_str());
//					elm_genlist_realized_items_update(self->__dialog->getEvasObj());
				},
				this,
				getEvasObj()
			);
...
void CalEditView::__onSave()
{
	...
	__workingCopy->getStart(dateTime);
	char buffer[255] = {0};
	dateTime.getStringParam(buffer);
	WDEBUG("AAAAA1 %s", buffer);
	WDEBUG("AAAAA2 hour %d, minutes %d", dateTime.getHour(), dateTime.getMinute());
	if(__time->isAllDay() &&
