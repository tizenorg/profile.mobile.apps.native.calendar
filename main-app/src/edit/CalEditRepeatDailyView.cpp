...
#define DATEFIELD_BUFFER 64
#define DATEFIELD_FORMAT "<p font_size=%d>%s</p>"

CalEditRepeatDailyView::CalEditRepeatDailyView(const CalScheduleRepeat repeatOri, const CalDateTime& dateTime, std::function<void (const CalScheduleRepeat repeat)> doneCb) :
	CalView("CalEditRepeatDailyView"),
	__doneCb(doneCb),
...
CalEditRepeatDailyView::~CalEditRepeatDailyView()
{
}

void CalEditRepeatDailyView::__createGenlist()
{
	Evas_Object *radio_main = elm_radio_add(__genlist);
		...
		CalDialogEditRepeatUnitItem *unitItem = new CalDialogEditRepeatUnitItem(
			...
			}, __repeat, CalScheduleRepeat::DAY);
...
Evas_Object* CalEditRepeatDailyView::onCreate(Evas_Object* parent, void* viewParam)
{
...
void CalEditRepeatDailyView::onPushed(Elm_Object_Item* naviItem)
{
	...
	evas_object_smart_callback_add(button, "clicked",
		[](void* data, Evas_Object* obj, void* event_info){
		CalEditRepeatDailyView *pView = (CalEditRepeatDailyView *)data;
...
void CalEditRepeatDailyView::__update()
{
...
void CalEditRepeatDailyView::onEvent(const CalEvent & event)
{
	WENTER();
