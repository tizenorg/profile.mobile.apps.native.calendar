	...
	__dateText()
{
	if(!__searchText.empty()) {
		...
		std::transform(__searchText.begin(), __searchText.end(),
				lowerSearchText.begin(), std::ptr_fun(tolower));
	...
	if(schedule) {
		...
		CalDateTime itemNextDay, groupDateTime(dayOfGroup.getYear(),
				dayOfGroup.getMonth(), dayOfGroup.getMday(), 0, 0, 0);
...
void CalEventItem::getHilightText(std::string &hilightText)
{
	...
	std::transform(hilightText.begin(), hilightText.end(), lowerText.begin(),
			std::ptr_fun(tolower));
