...
void CalMonthRowControl::resetDayNums(int firstWeekday, const CalDate& rowStartDate, int i)
{
	...
	for (int j = 0; j < DAYS_PER_WEEK; j++) {
		...
		snprintf(partname, PART_NAME_SIZE, "cell/%d/daynum", j);
		char text[TEXT_SIZE];
		snprintf(text, TEXT_SIZE, "%d", date.getMday());
...
{
	char partname[PART_NAME_SIZE];
	snprintf(partname, PART_NAME_SIZE, "cell/%d", j);
...
Evas_Object* CalMonthRowControl::__getContentObj()
{
	if (__content == NULL) {
		...
		if (__edjeGroupName)
			snprintf(groupName, GROUP_NAME_SIZE, "%s.content", __edjeGroupName);
		else
			strncpy(groupName, "CalMonthRowControl.content", GROUP_NAME_SIZE);
