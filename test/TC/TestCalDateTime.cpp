...
TEST_F(TestCalDateTime, ConstructWithIV)
{
	CalDateTime birthday(INIT_LOWER_BOUND);
	ASSERT_EQ(birthday.getYear(), LOWER_YEAR);
}

//____________________________________________________________________________//

TEST_F(TestCalDateTime, ConstructWithString)
{
	CalDateTime birthday("56756756756");
	ASSERT_GT(birthday.getYear(), 0);
}

//____________________________________________________________________________//

TEST_F(TestCalDateTime, dump)
{
	CalDateTime birthday(INIT_LOWER_BOUND);
	std::string tmpString = birthday.dump(true);
	ASSERT_TRUE(!tmpString.empty());
}

//____________________________________________________________________________//

TEST_F(TestCalDateTime, isSameMonth)
{
	CalDateTime birthday1(year, month, mday);
	CalDateTime birthday2(year, month, mday);
	ASSERT_TRUE(birthday1.isSameMonth(birthday2));
}

//____________________________________________________________________________//

TEST_F(TestCalDateTime, incrementDay)
{
	CalDateTime birthday(year, month, mday);
	birthday.incrementDay();
	ASSERT_EQ(birthday.getMday(), mday +1);
}

//____________________________________________________________________________//

TEST_F(TestCalDateTime, decrementDay)
{
	CalDateTime birthday(year, month, mday);
	birthday.decrementDay();
	ASSERT_EQ(birthday.getMday(), mday -1);
}

//____________________________________________________________________________//

TEST_F(TestCalDateTime, incrementMonth)
{
	CalDateTime birthday(year, month, mday);
	birthday.incrementMonth();
	ASSERT_EQ(birthday.getMonth(), month + 1);
}

//____________________________________________________________________________//

TEST_F(TestCalDateTime, decrementMonth)
{
	CalDateTime birthday(year, month, mday);
	birthday.decrementMonth();
	ASSERT_EQ(birthday.getMonth(), month - 1);
}

//____________________________________________________________________________//

TEST_F(TestCalDateTime, __normalizeStructTm)
{
	CalDateTime birthday(year, month, mday);
	struct tm time;
	birthday.__normalizeStructTm(time);

	ASSERT_EQ(time.tm_hour, 12);
}

//____________________________________________________________________________//

TEST_F(TestCalDateTime, __getWeekStartDate)
{
	CalDateTime birthday(year, month, mday);
	struct tm time;
	birthday.__getWeekStartDate(1 , time);
...
TEST_F(TestCalDateTime, compareMonth)
{
	CalDateTime birthday1(year, month, mday);
	CalDateTime birthday2(year, month +1, mday);

	int result = CalDateTime::compareMonth(birthday1.getYear(), birthday1.getMonth(), birthday2.getYear(), birthday2.getMonth());
...
TEST_F(TestCalDateTime, compareMonthWithDate)
{
	CalDateTime birthday1(year, month, mday);
	CalDateTime birthday2(year, month +1, mday);

	int result = CalDateTime::compareMonth(birthday1, birthday2);
...
TEST_F(TestCalDateTime, getWeekdayText)
{
	CalDateTime birthday(year, month, mday);
	const char * testString = birthday.getWeekdayText(1);
...
TEST_F(TestCalDateTime, getUnixTimeString)
{
	CalDateTime birthday(year, month, mday);
	const char * testString = birthday.getUnixTimeString();
	ASSERT_TRUE(!testString.empty());
}
...
{
	CalDateTime birthday(year, month, mday);
	const char * testString = birthday.getString();
...
TEST_F(TestCalDateTime, getMonthString)
{
	CalDateTime birthday(year, month, mday);
	const char * testString = birthday.getMonthString();
